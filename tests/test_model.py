...
Rule 14: Can the model overfit a single batch?
...
def test_overfit_single_batch():
    """Model can memorize a single batch (Rule 14)."""
    set_seed(42)
...
def test_dataloaders():
    ...
    set_seed(42)
    loaders = create_dataloaders(n_samples=100, batch_size=16, num_workers=0)
    assert len(loaders.train) > 0
    assert len(loaders.val) > 0


def test_loss_at_init():
    """Loss at initialization matches -log(1/n_classes) (Rule 13)."""
    set_seed(42)
    n_classes = 10
